...
import { announcements, calendarEvents, recognitions, type Announcement, type CalendarEvent, type Recognition } from '../data/mockData'
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 32, maxWidth: 1400 }}>
            ...
            <div className="animate-in" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <p style={{ fontSize: 15, color: '#64748B' }}>
                    {announcements.length} announcements · {calendarEvents.length} events · {recognitions.length} recognitions
                </p>
                <button className="btn btn-amber"><Plus style={{ width: 16, height: 16 }} />New Announcement</button>
            </div>

            {/* Summary cards */}
            <div className="animate-in" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
                {[
                    { label: 'Announcements', value: announcements.length, icon: Megaphone, bg: '#EFF6FF', tx: '#2563EB', sub: `${announcements.filter(a => a.type === 'memo').length} memos` },
            ...
            <div className="animate-in" style={{ display: 'flex', gap: 4, background: 'white', borderRadius: 16, padding: 4, boxShadow: '0 1px 3px rgba(15,23,42,0.04)', width: 'fit-content' }}>
                {([
                    { key: 'announcements' as TabView, label: `Announcements (${announcements.length})` },
